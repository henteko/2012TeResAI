...
/**********************************************************
	初期化時に呼ばれる関数
**********************************************************/
void TsuuroInit(AI_T &myAi)
{
	myAi.Graph = LoadGraph("AI_image\\tsuuro.png");  //画像の設定
	strcpy_s(myAi.name, "Tsuuro");  //自分のAIの名前設定
}
...
Action Tsuuro(int view[2*VISIBLE+1][2*VISIBLE+1])
{
	int danger=0;
	int tx,ty;
	...
	for(int i=36+Sx;i<36+Sx+2*VISIBLE+1;i++){
		for(int j=36+Sy;j<36+Sy+2*VISIBLE+1;j++){
			map[i][j]= view[i][j];
